...
public interface MuestraRepositorio extends PagingAndSortingRepository<Muestra, Integer> {
	...
	@Query("SELECT muestra FROM Muestra muestra "
			...
			+ "JOIN muestra.pacientes pacientesMuestra "
			+ "WHERE 1=1 and "
			+ "(:#{#params.idCentro} is null or centro.id = :#{#params.idCentro}) and "
			+ "(:#{#params.criterioNombre} is null or pacientesMuestra.nombrePaciente like :#{#params.criterioNombre}) and "
			+ "(:#{#params.criterioPrimerApellido} is null or pacientesMuestra.apellido1paciente like :#{#params.criterioPrimerApellido}) and "
			+ "(:#{#params.criterioSegundoApellido} is null or pacientesMuestra.apellido2paciente like :#{#params.criterioSegundoApellido}) and "
			+ "(:#{#params.criterioNHC} is null or pacientesMuestra.nhc like :#{#params.criterioNHC}) and "
			+ "(:#{#params.criterioCorreo} is null or pacientesMuestra.email like :#{#params.criterioCorreo}) and "
