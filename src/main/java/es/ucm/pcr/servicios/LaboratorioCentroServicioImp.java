...
import es.ucm.pcr.beans.MuestraListadoPlacasLaboratorioBean;
...
public class LaboratorioCentroServicioImp implements LaboratorioCentroServicio {
	...
	public LaboratorioCentro mapeoBeanEntidadLaboratorioCentro(BeanLaboratorioCentro beanLaboratorioCentro)
			throws Exception {
	...
	public BeanLaboratorioCentro mapeoEntidadBeanLaboratorioCentro(LaboratorioCentro laboratorioCentro)
			throws Exception {
...
//					"L"));
	...
	public Map<Integer, String> mapaLaboratoriosCentro(List<BeanLaboratorioCentro> laboratoriosCentro)
			throws Exception {
	...
	public Page<PlacaLaboratorioCentroBean> buscarPlacas(BusquedaPlacaLaboratorioBean criteriosBusqueda,
		...
		Page<PlacaLaboratorio> PagePlacasLaboratorioCentro = placaLaboratorioRepositorio.findByParams(criteriosBusqueda,
				pageable);
		...
		Page<PlacaLaboratorioCentroBean> placasLaboratorioCentro = new PageImpl<>(listaPlacasLaboratorioCentroBean,
				pageable, PagePlacasLaboratorioCentro.getTotalElements());
	...
	public PlacaLaboratorioCentroBean crearPlaca(PlacaLaboratorioCentroBean placaLaboratorioCentroBean) {
		...
		// Transformamos en un Array el String que viene de la vista con los IDs de las
		// placas Visavet seleccionadas
		String[] listaIDsPlacasVisavetSeleccinadas = placaLaboratorioCentroBean.getPlacasVisavetSeleccionadas()
				.split(":");
	...
	public void finalizarPCR(Integer id) {
		...
		if (placa.isPresent()) {
			if (placa.get().getEstadoPlacaLaboratorio().getId() == Estado.PLACA_PREPARADA_PARA_PCR.getCodNum()) {
				placa.get()
						.setEstadoPlacaLaboratorio(new EstadoPlacaLaboratorio(Estado.PLACA_FINALIZADA_PCR.getCodNum()));
	...
	public void asignarEquipoPCR(Integer id) {
		...
		if (placa.isPresent()) {
			if (placa.get().getEstadoPlacaLaboratorio().getId() == Estado.PLACA_INICIADA.getCodNum()) {
				...
				placa.get().setEstadoPlacaLaboratorio(
						new EstadoPlacaLaboratorio(Estado.PLACA_PREPARADA_PARA_PCR.getCodNum()));
				placaLaboratorioRepositorio.save(placa.get());
			}
		}
	}
	...
	public PlacaLaboratorioCentroBean guardarCogerODevolverPlaca(Integer idPlaca, Integer idUsuario, String accion) {
		...
		if (accion.equals("coger")) {
			// asocia la placa al usuario, le cambia el estado de la placa a
			// PLACA_ASIGNADA_PARA_ANALISIS y pone a todas sus muestras a estado pendente de
			// analizar
			Usuario usuario = usuarioRepositorio.getOne(idUsuario);
			placa.setUsuario(usuario);
			EstadoPlacaLaboratorio estadoPlacaLab = estadoPlacaLaboratorioRepositorio
					.getOne(BeanEstado.Estado.PLACA_ASIGNADA_PARA_ANALISIS.getCodNum());
			...
			// recorremos todas las muestras de esa placa y les ponemos el estado pendente
			// de analizar
			...
			// desasocia la placa del usuario, le cambia el estado de la placa a
			// PLACA_LISTA_PARA_ANALISIS (estado anterior) y ¿Qué hacemos con las muestras?
			placa.setUsuario(null);
			EstadoPlacaLaboratorio estadoPlacaLab = estadoPlacaLaboratorioRepositorio
					.getOne(BeanEstado.Estado.PLACA_LISTA_PARA_ANALISIS.getCodNum());
			System.out.println("el estado que le vamos a asignar a la placa es: " + estadoPlacaLab.getDescripcion());
			placa.setEstadoPlacaLaboratorio(estadoPlacaLab);
			// TODO preguntar que hacemos con las muestras cuando devolvermos la placa?
			// recorremos todas las muestras de esa placa y les ponemos en el estado que
			// tenian al llegar MUESTRA_ENVIADA_CENTRO_ANALISIS?
			for (Muestra m : placa.getMuestras()) {
		...
		placa = placaLaboratorioRepositorio.save(placa);
		// TODO guardamos en el log el cambio de estado de la placa (no tenemos tabla de
		// log de placas)
	...
	public List<PlacaLaboratorioCentroBean> buscarPlacasAsignadasAJefe(Usuario usuario) {
		// busca la lissta de placas que se ha asignado el Jefe (que estan bajo su
		// responsabilidad)
	...
	// metodo que guarda las nuevas asignaciones de analistas de labCentro,
	// voluntarios de labCentro y voluntarios sin centro a la placa
	// se guardan en realidad las asignaciones a las muestras de la placa
	@Override
	@Transactional
	public void guardarAsignacionesAnalistasYVoluntariosAPlacaYmuestras(
			GuardarAsignacionPlacaLaboratorioCentroBean formBeanGuardarAsignacionPlaca) {
		...
		if (formBeanGuardarAsignacionPlaca.getListaIdsAnalistasLabSeleccionados().size() > 0
			...
			// recorremos todas las muestras de esa placa, les asignamos los nuevos
			// analistas, aumentamos el contador de analistas asignados
			// y les ponemos el estado asignada analista
			for (Muestra muestra : placa.getMuestras()) {
				Integer numerodeAnalistasAsignadosMuestra = (muestra.getNumerodeAnalistasAsignados() == null) ? 0
						: muestra.getNumerodeAnalistasAsignados();
				// creamos nuevos usuarioMuestras por cada nueva asignacion de cada una de las
				// muestras
				// recorremos los analistas de labaratorio marcados para asignar
				for (Integer idAnalistaLabSeleccionado : formBeanGuardarAsignacionPlaca
						.getListaIdsAnalistasLabSeleccionados()) {
				...
				for (Integer idAnalistaVolSeleccionado : formBeanGuardarAsignacionPlaca
						.getListaIdsAnalistasVolSeleccionados()) {
				...
				for (Integer idOtroVolSeleccionado : formBeanGuardarAsignacionPlaca
						.getListaIdsVolSinLabCentroSeleccionados()) {
			...
			System.out.println(
					"no se ha escogido ningun analista nuevo, no hacemos asignaciones nuevas ni cambiamos el estado a sus muestras");
		}

		// a la placa no hay que cambiarle el estado, solo cambiamos el estado de sus
		// muestras
	...
	private Boolean muestraResuelta(Muestra m) {
		...
		// una muestra está resuelta si tiene estado resuelta, tiene resultado y fecha
		// de resultado
		if (m.getEstadoMuestra().getId() == Estado.MUESTRA_RESUELTA.getCodNum() && m.getResultado() != null
				&& !m.getResultado().isEmpty() && m.getFechaResultado() != null) {
			resuelta = true;
		}
		return resuelta;
	}

	// metodo que nos dirá si las muestras de la placa pasada por parámetro tiene un
	...
	public Boolean tienenResultadoDefinitivoLasMuestrasDeLaPlaca(Integer idPlaca) {
		...
		if (placa.getMuestras().size() == 0) {
			...
			// es decir no tiene el estado resuelta, no tiene resultado o no tiene fecha de
			// resultado) devolvemos false
			for (Muestra m : placa.getMuestras()) {
	...
	@Override
	public Page<PlacaLaboratorioCentroAsignacionesAnalistaBean> buscarPlacas(
			BusquedaPlacaLaboratorioAnalistaBean criteriosBusqueda, Pageable pageable) {
		...
		Page<PlacaLaboratorio> PagePlacasLaboratorioCentro = placaLaboratorioRepositorio
				.findByParamsValoradasAndNotValoradas(criteriosBusqueda, pageable);
		for (PlacaLaboratorio placa : PagePlacasLaboratorioCentro.getContent()) {
			System.out.println("Placa vale: " + placa.toString());
			PlacaLaboratorioCentroAsignacionesAnalistaBean placaLaboratorioCentroAsignacionesAnalistaBean = PlacaLaboratorioCentroAsignacionesAnalistaBean
					.modelToBean(placa, criteriosBusqueda.getIdAnalistaMuestras());
			listaPlacasLaboratorioCentroAsignacionesBean.add(placaLaboratorioCentroAsignacionesAnalistaBean);
		}
		Page<PlacaLaboratorioCentroAsignacionesAnalistaBean> placasLaboratorioCentroAsignacionesAnalista = new PageImpl<>(
				listaPlacasLaboratorioCentroAsignacionesBean, pageable, PagePlacasLaboratorioCentro.getTotalElements());
		return placasLaboratorioCentroAsignacionesAnalista;
	}

	private Integer calculaCuantasValoracionesTieneLaMuestra(Muestra m) {
		// metodo que nos dice cuantas valoraciones han dado los analistas asignados a
		// la muestra
		// de una muestra recuperamos todos sus analistas asignados y contamos cuantos
		// han puesto una valoracion
		Integer cont = 0;
	...
	@Override
	@Transactional
	// metodo que recibe el ElementoDocumentacionBean (excel con valoraciones) y
	// carga las valoraciones que ha metido el analista en todas las muestras de la
	// placa
	public void guardarResultadosPlacaLaboratorio(ElementoDocumentacionBean bean, Integer numAnalistas) throws Exception {
		...
		Integer idPlaca = bean.getId(); // aqui solo puede llegar el id de una placa del analista logado que tenga sus
										// muestras sin valorar
		PlacaLaboratorio placa = placaLaboratorioRepositorio.getOne(idPlaca);
		...
		for (Muestra m : placa.getMuestras()) {
			String valoracionExcelParaMuestra = resultados.get(m.getId()); // TODO, aquí habría que recoger la
																			// valoracion del excel para la muestra,
																			// esto es un ejemplo
			// recuperamos el usuarioMuestra asociado al usuario logado (analista que está
			// valorando las muestras de la placa)
			Optional<UsuarioMuestra> optusumu = usuarioMuestraRepositorio.findByIdUsuarioAndIdMuestra(idUsuarioLogado,
					m.getId());
			if (optusumu.isPresent()) {
				UsuarioMuestra usumu = optusumu.get();
				// aqui tendriamos que asociarle la valoracion que viene en el excel para esa
				// muestra
				...
				log.error(
						"No se ha encontrado el registro de usuario muestra para asignarle la valoracion del analista");
			}

			// por cada muestra calculamos si ya se han dado las valoraciones de todos los
			// analistas
			// en ese caso el resultado sería definitivo y se guardara el resultado en la
			// muestra y se enviará la notificacion
			Integer numValoracionesMuestra = this.calculaCuantasValoracionesTieneLaMuestra(m);
			System.out.println("el numero de valoraciones actuales de la muestra con id: " + m.getId() + " es: "
					+ numValoracionesMuestra);
			if (numValoracionesMuestra.equals(numAnalistas)) {
				// guardamos como resultado definitivo este ultimo resultado que está dando el
				// analista que viene del excel
				...
				estadoMuestra.asignarTipoEstadoYCodNum(TipoEstado.EstadoMuestra, m.getEstadoMuestra().getId());
				servicioLog.actualizarEstadoMuestra(m.getId(), estadoMuestra);
				// Envio de notificacion al pacientes si tiene activada la notificacion
				// automatica
				// recupero el paciente de la muestra
				Paciente paciente = m.getPaciente();
				System.out.println("el paciente con id: " + paciente.getId() + " tiene la notificacion automato a: "
						+ paciente.getNotificarAutomato());
				if (paciente.getNotificarAutomato().equals("S")) {
					muestraServicio.actualizarNotificacionMuestra(m.getId(), true); // actualiza fecha de notificación y
																					// envia correo
				}
	...
	public BeanLaboratorioCentro buscarLaboratorioById(Integer id) {
		...
		if (laboratorioOp.isPresent()) {
			...
			return new BeanLaboratorioCentro(laboratorioCentro.getId(), laboratorioCentro.getNombre(),
					laboratorioCentro.getDocumentos(), laboratorioCentro.getPlacaLaboratorios(),
					laboratorioCentro.getEquipos(), "L");
		} else
			return null;
	}
